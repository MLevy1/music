...
import {ROOT,clip,edit,move,validate,copyCell,pasteCell,fillRegion,cellFromCoordinates,hasDetailedOccupancy,validatePatterns} from "./space.mjs";
...
<button id="fill-region" class="button">Fill rectangle</button>
<p class="storage-note">Uses the active object and selected color. Other objects block the fill.</p></details>`;
panel.after(bulk);
...
function requireSelection(){if(!selected)throw Error("Select a starting cell first.");if(isolated)throw Error("Turn off the isolated view before pasting or filling.");}
...
$("#fill-region").onclick=()=>run(()=>{
 requireSelection();const unit=Number($("#fill-unit").value);
...
const viewShortcuts=document.createElement("div");viewShortcuts.className="view-shortcuts";viewShortcuts.setAttribute("aria-label","Standard views");viewShortcuts.innerHTML='<button type="button" data-view="front">Front</button><button type="button" data-view="top">Top</button><button type="button" data-view="side">Side</button>';$(".viewport-wrap").append(viewShortcuts);
...
function standardView(name){
 ...
 if(name==="side")camera.position.set(18,5,0);
